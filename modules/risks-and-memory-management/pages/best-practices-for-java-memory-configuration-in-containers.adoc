...
As Java has evolved to be container-aware, the manual strategy of defining absolute heap sizes (e.g., `-Xmx2g`) has become a deployment anti-pattern:
*   Xmx direct usage: in most cases, it is an anti-pattern.
*   JVM arguments directly at build time, rather than runtime.
