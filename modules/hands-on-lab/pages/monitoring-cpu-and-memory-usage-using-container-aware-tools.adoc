...
:page-audio-src: monitoring-cpu-and-memory-usage-using-container-aware-tools.wav
:page-transcript-src: monitoring-cpu-and-memory-usage-using-container-aware-tools.txt
...
== The Visibility Gap
If you use standard Linux utilities such as `lscpu`, `nproc`, or `free -m` inside a container, you are likely seeing the resources of the underlying host node. Relying on these values for capacity planning and testing likely will lead to OOMExceptions (OOMException), Out of Memory (OOM) kills or CPU throttling because the JVM may attempt to allocate resources exceeding the container's cgroup limits.
...
Usage of Java Diagnostics (jcmd) is always preffered. 
...
When running a container-aware command (such as jcmd's VM.info's and `java -XshowSettings:system -version`), look for the following key indicators:
...
*   **active processor`**: this shows how many CPU were detected.
...
jcmd <pid> VM.info
----
2. Search for the `container` section in the output (cgroup). This provides a real-time snapshot of the container metrics, including `memory_usage_in_bytes`. Use this to determine if your heap size (typically a percentage of the limit) is appropriately leaving enough headroom for off-heap memory.
