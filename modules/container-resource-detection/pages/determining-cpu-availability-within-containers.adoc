...
- For cgroups v2, versions Java 8u372+ Java 11.0.16+ are required.
...
In this short task, you will verify the JVM's CPU detection. To make this work we will need an container image with an application already there, otherwise the container will fail on the spot.

. **Create an image container with the application:**
+
Use the following command to fetch a project and build a OpenJDK project locally from it via s2i toolkit:
+
[source,bash]
----
...
. Deployment in Openshift  via new project, one can use the binary upload:
...
* Broken values are rounded to the lowest integer, so 2200 millicores, becomes 2 CPU.
