...
*   **Operational Benefits:** Accurate detection prevents OutOfMemory and OutOfMemoryKill  caused by the JVM incorrectly defaulting to host-level physical memory rather than container-specific limits.

In simple terms: do not not worry about the Xmx directly vs size of the container. Instead, focus on the adequate correct percentage value for heap inside a container and how the application behaves.
